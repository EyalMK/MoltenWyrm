...
namespace LuaGlobalFunctions
{
    ...
    int GetPlayersInWorld(lua_State* L)
    {
        ...
        uint32 i = 0;
        {
			...
			// TODO : Weird
			const HashMapHolder<Player>::LockType* g = HashMapHolder<Player>::GetLock();
	...
	int IsSanctuary(lua_State* L)
	{
		...
		Eluna::Push(L, areaEntry->IsSanctuary());
		return 1;
	}
  ...
  int GetActiveGameEvents(lua_State* L)
  {
	...
	// SKULL : Added custom hooks
	int RegisterCustomEvent(lua_State* L)
	{
		return RegisterEventHelper(L, Hooks::REGTYPE_CUSTOM);
	}
    ...
    int PerformIngameSpawn(lua_State* L)
    {
        ...
        if (spawntype == 1) // spawn creature
        {
            if (save)
            {
                ...
                if (!creature->Create(creature->GetGUID(), map, phase, entry, x, y, z, o, 0, 0 ))
                {
                    delete creature;
                    Eluna::Push(L);
                    return 1;
                ...
                uint32 db_guid = creature->GetGUID();
        ...
        if (spawntype == 2) // Spawn object
        {
            ...
            uint32 guidLow = object->GetGUID();

            //G3D::Quat rot = G3D::Matrix3::fromEulerAnglesZYX(o, 0.f, 0.f);

            if (!object->Create(guidLow, objectInfo->entry, map, phase, x, y, z, o, 0, 0, 0, 0, 0, GO_STATE_READY))
            {
                delete object;
                Eluna::Push(L);
                return 1;
