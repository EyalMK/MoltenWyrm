...
void Unit::Update(uint32 p_time)
{
    ...
    // update combat timer also for npc_bots
...
uint32 Unit::DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
    ...
    if (GetTypeId() != TYPEID_PLAYER)
        if (victim->HasAura(93099) || victim->HasAura(84839) || victim->HasAura(93098)) // Vengeance
        {
            ...
            if (AuraEffect* vng = victim->GetAuraEffect(76691, EFFECT_0))
    ...
    if (GetTypeId() != TYPEID_PLAYER)
        if (victim->HasAura(84840) && victim->HasAura(5487)) // Vengeance Feral (Bear)
        {
...
// The emote opcode is used only for ONESHOT animations now, and UNIT_NPC_EMOTESTATE for states.
...
void Unit::AttackerStateUpdate (Unit* victim, WeaponAttackType attType, bool extra)
{
    ...
    else
    {
        ...
        {
            AuraEffectList const& mTotalAuraList = GetAuraEffectsByType(SPELL_AURA_PROC_TRIGGER_SPELL_2);
            for (AuraEffectList::const_iterator i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
            {
                int32 trigger = (*i)->GetSpellInfo()->Effects[(*i)->GetEffIndex()].TriggerSpell;
                CastSpell(victim,trigger,true);
                return;
            }
            ...
            for (AuraEffectList::const_iterator i = _TotalAuraList.begin(); i != _TotalAuraList.end(); ++i)
            {
                if ((*i)->GetId() == 81330 || (*i)->GetId() == 81332 || (*i)->GetId() == 81333)
                    if (!ToPlayer() || !ToPlayer()->GetWeaponForAttack(BASE_ATTACK) || !ToPlayer()->IsUseEquipedWeapon(true) || !ToPlayer()->GetWeaponForAttack(BASE_ATTACK)->IsFitToSpellRequirements((*i)->GetSpellInfo()))
                        continue;

                damageInfo.damage += (damageInfo.damage * (*i)->GetAmount()) / 100;
            }
        }
...
void Unit::SendAuraVisualForSelf(bool apply, uint32 id, uint8 effmask)
{
    ...
    data.appendPackGUID(ToPlayer()->GetGUID());
    // use always free 64+ slots
    data << uint8(MAX_AURAS);// slot
    if (!apply)
    {
        data << uint32(0);
        SendMessageToSet(&data, true);
    ...
    data << uint32(id); //Aur id
    data << uint16(AFLAG_CASTER | AFLAG_POSITIVE | effmask); // Aur flags
    data << uint8(getLevel()); // caster level
    data << uint8(1); // charge
    SendMessageToSet(&data, true);
}
...
{
    bool _isAutoShot = m_currentSpells[CURRENT_AUTOREPEAT_SPELL]->m_spellInfo->Id == 75;
    // check "realtime" interrupts
    if (((GetTypeId() == TYPEID_PLAYER && ToPlayer()->isMoving()) || IsNonMeleeSpellCast(false, false, true, _isAutoShot)) && !HasAuraTypeWithAffectMask(SPELL_AURA_CAST_WHILE_WALKING, m_currentSpells[CURRENT_AUTOREPEAT_SPELL]->m_spellInfo))
    {
        // cancel wand shoot
        if (!_isAutoShot)
    ...
    if (m_AutoRepeatFirstCast && getAttackTimer(RANGED_ATTACK) < 500 && !_isAutoShot)
...
bool Unit::CanCastWhileWalking(uint32 spell_id)
{
    ...
    if(HasAura(86914)) // Firestarter mage
       if(spell_id == 2948)
          return true;

    if(HasAura(82661)) // Aspect of the Fox hunter
    {
       if(spell_id == 56641)
          return true;
    ...
    switch (dummySpell->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (dummySpell->Id)
            {
                ...
                case 85466:
                {
                    if (Unit *targetBaneOfHavoc = triggeredByAura->GetCaster())
                    {
                        if (targetBaneOfHavoc->GetGUID() == target->GetGUID() ||!targetBaneOfHavoc->HasAura(80240))
                            return false;
                        int32 bp = damage * 0.15;
                        CastCustomSpell(targetBaneOfHavoc,85455,&bp,NULL,NULL,true);
                    }
                    return true;
                }
                // Unstable Power
        ...
        {
            // Fel Armor
            if (dummySpell->Id == 28176)
            {
                basepoints0 = int32(damage * triggerAmount / 100);
                CastCustomSpell(this, 96379, &basepoints0, NULL, NULL, true);
                break;
            }
            // Seed of Corruption
            ...
            switch (dummySpell->Id)
            {
                ...
                case 92297:
                {
                    if (procSpell->Id == 585)
                    {
                        ...
                        {
                            if (caster->HasSpellCooldown(47540) && ((HasAura(92295) && roll_chance_i(50)) || (HasAura(92297))))
                            {
                    ...
                    if (procSpell->Id == 2060)
                    {
                        if (Player* caster = triggeredByAura->GetCaster()->ToPlayer())
                        {
                            if (caster->HasSpellCooldown(89485) && ((HasAura(92295) && roll_chance_i(50)) || (HasAura(92297))))
                            {
        ...
        case SPELLFAMILY_ROGUE:
        {
            switch (dummySpell->Id)
            {
                ...
                case 56807:
                {
                    if (procSpell->SpellIconID != 153)
                        return false;
                    basepoints0 = (triggerAmount * damage / 100) / 8;
                ...
                case 79134:
                {
                    if (!procSpell || (procSpell->Id != 1943 && procSpell->Id != 703))
                        return false;

                    bool IsPoisoned = false;
                    Unit::AuraApplicationMap appliedAuras = victim->GetAppliedAuras();
                    for (Unit::AuraApplicationMap::iterator iter = appliedAuras.begin(); iter != appliedAuras.end(); ++iter)
                        if (iter->second->GetBase()->GetSpellInfo()->Dispel == DISPEL_POISON)
                            IsPoisoned = true;

                    if (!IsPoisoned)
                        return false;

                    CastSpell(victim, 79136, false);
                    int32 bp = 10;
                    CastCustomSpell(this, 51637, &bp, NULL, NULL, true);
                }
                    ...
                    if (Player* pTarget = triggeredByAura->GetCaster()->ToPlayer())
                    {
                        ...
                        if (procEx & PROC_EX_CRITICAL_HIT)
                        {
                            ...
                            if (spellTarget && pTarget->IsValidAttackTarget(spellTarget))
                            {
                                ...
                                pTarget->AddSpellCooldown(51699, 0, time(NULL) + 2);
                            }
                        }
                    }
                    break;
            ...
            switch (dummySpell->SpellIconID)
            {
                ...
                {
                    if (!procSpell || (procSpell->Id != 1752 && procSpell->Id != 57293))
                        return false;

                    int32 Damagebp0 = 0;
                    int32 hitAmount = 1;

                    if (!victim->HasAura(84748, GetGUID()))
                    {
                        CastCustomSpell(victim, 84748, &Damagebp0, &hitAmount, NULL, true);
                    ...
                    else if (victim->HasAura(84748, GetGUID()))
                    {
                        hitAmount = hitAmount == 4 ? 4 : victim->GetAura(84748)->GetEffect(EFFECT_1)->GetAmount() + 1;
                        ...
                        if (hitAmount == 4)
                        {
                            ...
                            if (HasAura(84745))
                            {
                                ...
                                AddAura(84746, this);
                                Damagebp0 = 20;
                                victim->GetAura(84748)->RefreshDuration();
                            }
                            else if (HasAura(84746))
                            {
                                RemoveAura(84746);
                        ...
                        if (Damagebp0 != 0)
                            victim->GetAura(84748)->GetEffect(EFFECT_0)->ChangeAmount(Damagebp0);
                        victim->GetAura(84748)->GetEffect(EFFECT_1)->ChangeAmount(hitAmount);
                    }
                }
            }
            break;
        ...
        case SPELLFAMILY_HUNTER:
        {
            ...
            // Aspect of the Fox: Focus bonus
            if (dummySpell->Id == 82661)
            {
                uint32 basepoints = 0;
        ...
        case SPELLFAMILY_PALADIN:
        {
            switch (dummySpell->Id)
            {
                ...
                case 20164:
                {
                    if (procSpell && (procSpell->Id == 54158 || procSpell->Id == 24275))
                        return false;
                    break;
                }
                ...
                case 86674:
                {
                    ...
                    if (!target)
                        return false;
                    int32 bp = damage;
                    int32 bp2 = damage * 10 / 100;
                    target->CastCustomSpell(victim, 86678, &bp, &bp2, NULL, true);
                    return true;
                }
                // Guardian of ancient king
                case 86701:
        ...
        case SPELLFAMILY_SHAMAN:
        {
            switch (dummySpell->Id)
            {
                ...
                case 77222:
                {
                   ...
                   if (!roll_chance_i(GetAura(77222)->GetEffect(0)->GetAmount()))
                       return false;
            ...
            if (dummySpell->SpellIconID == 4674)
            {
                ...
                int32 mana = -(manacost * triggeredByAura->GetAmount()) / 100;
                int32 effect = triggeredByAura->GetAmount();

                CastCustomSpell(victim, 77800, &mana, &effect, &effect, true, 0, 0, GetGUID());
                return true;
            }
            // Static Shock
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            {
                if (!procSpell || (procSpell->Id != 45462 && procSpell->Id != 45477 && procSpell->Id != 45477 && procSpell->Id != 45524 && procSpell->Id != 50304))
                    return false;

                int32 basepoints = triggerAmount;
                CastCustomSpell(victim, 65142, &basepoints, NULL, NULL, true);
                break;
            }
            // Runic Empowerment
            if (dummySpell->Id == 81229)
            {
                if (AuraEffect const * aurEff = GetDummyAuraEffect(SPELLFAMILY_DEATHKNIGHT, 4068, 0))
                {
                    int32 basepoints = aurEff->GetAmount();
                    CastCustomSpell(this, 51460, &basepoints, NULL, NULL, true);
                }
                else
                {
                    if (Player* player = ToPlayer())
...
bool Unit::HandleAuraProc(Unit* victim, uint32 damage, Aura* triggeredByAura, SpellInfo const* procSpell, uint32 /*procFlag*/, uint32 procEx, uint32 cooldown, bool * handled)
{
    ...
    switch (dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if (dummySpell->Id == 20066)
             ...
             if (dummySpell->Id == 31801)
             {
                if (procSpell && (procSpell->Id == 2812 || procSpell->Id == 31804 || procSpell->Id == 24275 || procSpell->Id == 26573 || procSpell->Id == 31935 || procSpell->Id == 53385))
                    return false;
        ...
        case SPELLFAMILY_MAGE:
        {
            switch (dummySpell->Id)
            {
                ...
                case 12571:
                {
                      Unit* target = GetGuardianPet();
                      if (!target)
                          return false;
                      int32 basepoints0 = damage * triggeredByAura->GetEffect(0)->GetAmount() / 100;
                      CastCustomSpell(target,91394,&basepoints0, NULL,NULL,true);
                      return true;
                }
            }
            break;
    ...
    // Dark intent - proc
    if (triggeredByAura->GetId() == 85767 || triggeredByAura->GetId() == 85768)
    {
        if (!(procEx & PROC_EX_CRITICAL_HIT))
            return false;

        int32 bp = triggeredByAura->GetId() == 85767 ? 4 : 1;
        CastCustomSpell(triggeredByAura->GetCaster(), 85759, &bp, &bp, NULL, true);
    }
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
                ...
                switch (auraSpellInfo->Id)
                {
                    ...
                    case 86304:
                        if (HealthAbovePct(50))
                            return false;
                        if (ToPlayer()->HasSpellCooldown(11426) || HasAura(11426))
                            return false;

                        cooldown = 30;
                        if (AuraEffect const* aurEff = GetAuraEffect(SPELL_AURA_ADD_PCT_MODIFIER, SPELLFAMILY_MAGE, 2133, 0))
                    ...
                    case 96887: // Item - Lightning Capacitor Normal ( FireLands)
                    {
                        ...
                        switch (auraSpellInfo->Id)
                        {
                            ...
                            case 97119: // Item - Lightning Capacitor Heroic( FireLands)
                                ...
                                break;
                            case 96887: /// @todo find right trigger
                                stack_spell_id = 96890;
                                trigger_spell_id = 96891;
                                break;
                        }
    ...
    switch (auraSpellInfo->Id)
    {
        ...
        case 32215:
        {
            if (GetTypeId() != TYPEID_PLAYER || !ToPlayer()->isHonorOrXPTarget(victim))
                return false;
            break;
        }
        // Bloodworms
        ...
        case 85126:
        {
            ...
            if (procSpell && (procSpell->Id == 54158 || procSpell->Id == 31804 || procSpell->Id == 20187 || procSpell->Id == 24275))
                return false;
        ...
        case 79684:
        {
            ...
            if (GetAuraEffect(SPELL_AURA_PROC_TRIGGER_SPELL, SPELLFAMILY_MAGE, 2938, 0))
                return false;
            if (GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_MAGE, 2999, 0))
                return false;
    ...
    switch (trigger_spell_id)
    {
        ...
        case 91342:
        {
            if (!procSpell || (procSpell->Id != 47632 && procSpell->Id != 47633))
                return false;
        ...
        case 16236:
        {
            int32 hp = damage * 10 / 100;
            if (hp > int32(victim->CountPctFromMaxHealth(10)))
                hp = victim->CountPctFromMaxHealth(10);
            CastCustomSpell(victim,105284,&hp,NULL,NULL,true);
        ...
        case 91021:
        {
            if (!procSpell || (procSpell->Id != 8676 && procSpell->Id != 703 && procSpell->Id != 1833))
                return false;
            break;
        }
        ...
        case 45241: // Blessed Resilience
        case 45242: // Blessed Resilience
        case 33143: // Focused Will
        case 89007: // Masochism
        {
            ...
            if (trigger_spell_id == 89007)
            {
                ...
                {
                    CastCustomSpell(ToPlayer(), trigger_spell_id, &triggerAmount, NULL, NULL, true);
                    return true;
                }
                else
                    return false;
            ...
            {
                if (proc || (procSpell && procEx == PROC_EX_CRITICAL_HIT && !procSpell->IsPeriodicDamage())) 
                {
                    CastCustomSpell(ToPlayer(), trigger_spell_id, &triggerAmount, NULL, NULL, true);
                    return true;
                }
...
void Unit::SetEclipsePower(int32 power)
{
    ...
    if (eclipse >= 100)
    {
        ...
        if (HasAura(93401))
            AddAura(94338, ToPlayer());

        // Euphoria
        if (AuraEffect const* pAurEff = GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_DRUID, 4431, 2))
        {
            int32 bp = pAurEff->GetAmount();
            CastCustomSpell(ToPlayer(), 81070, &bp, NULL,NULL,true);
        }
    }
...
uint32 Unit::SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
            ...
            case 911: // Shatter
            {
                if (spellProto->Id == 116)
                    if (victim->HasAuraState(AURA_STATE_FROZEN, spellProto, this))
                        if (((*i)->GetId() == 11170 || (*i)->GetId() == 12982) && (*i)->GetBase()->HasEffect(EFFECT_1))
                            AddPct(DoneTotalMod, (*i)->GetBase()->GetEffect(EFFECT_1)->GetAmount());
                break;
            }
    ...
    switch (spellProto->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
            ...
            if (spellProto->Id == 86704)
                if (HasAura(86700))
                {
                    DoneTotalMod *= GetAura(86700)->GetStackAmount();
                    RemoveAurasDueToSpell(86700);
                }
            break;
        case SPELLFAMILY_MAGE:
            // Frostburn
            if (AuraEffect* aurEff = GetAuraEffect(76613, 0))
                if (victim->HasAuraState(AURA_STATE_FROZEN, spellProto, this) || (spellProto->Id == 30455 && HasAura(44544)))
            ...
            //Arcane mastery
            if (AuraEffect const * aurEff = GetAuraEffect(76547, EFFECT_0))
            {
                int32 bp = aurEff->GetAmount();
                if (GetPower(POWER_MANA) < GetMaxPower(POWER_MANA) / 2)
                    bp = bp / 2;
        ...
        case SPELLFAMILY_PRIEST:
            ...
            if (spellProto->Id == 8092 || spellProto->Id == 73510)
                if (HasAura(77487))
                {
                    uint32 MasteryPct = 0;
                    if (HasAura(77486))
                        MasteryPct = GetAura(77486)->GetEffect(0)->GetAmount();
                    uint8 stack = GetAura(77487)->GetStackAmount();
                    float pct = 0;
                    for (uint8 i = 1; i <= stack; i++)
                        pct += 22 + MasteryPct;

                    DoneTotalMod *= (pct + 100.0f) / 100.0f;
                    RemoveAurasDueToSpell(77487);
                    CastSpell(ToPlayer(),95799,true);
                }
            ...
            //Shadow word: Death
            if (spellProto->SpellFamilyFlags[1] & 0x2)
                //When target has 25% pct of max health damage *3
                if (victim->HealthBelowPct(25))
                {
                    DoneTotalMod *= 3;
                    //Mind Melt
                    if (AuraEffect * aurEff = GetDummyAuraEffect(SPELLFAMILY_PRIEST, 3139, 0))
                        DoneTotalMod += aurEff->GetAmount()/100;
                }
            break;
        case SPELLFAMILY_WARLOCK:
            // Fire and Brimstone
...
uint32 Unit::SpellDamageBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack) const
{
    ...
    for (AuraEffectList::const_iterator i = mDummyAuras.begin(); i != mDummyAuras.end(); ++i)
    {
        switch ((*i)->GetSpellInfo()->SpellIconID)
        {
            ...
            // Inner Sanctum
            case 51:
                if ((*i)->GetMiscValue() == 12 || (*i)->GetMiscValue() == 23)
                {
                    if (GetTypeId() != TYPEID_PLAYER || !HasAura(588))
                        continue;
                    AddPct(TakenTotalMod, ((*i)->GetAmount() * (-1)));
                }
                break;
        }
    }
...
int32 Unit::SpellBaseDamageBonusDone(SpellSchoolMask schoolMask) const
{
    ...
    // Now only 30814
    AuraEffectList const& OnlyAttack = GetAuraEffectsByType(SPELL_AURA_OVERRIDE_SPELL_POWER_BY_AP_PCT);
    for (AuraEffectList::const_iterator i = OnlyAttack.begin(); i != OnlyAttack.end(); ++i)
    {
    ...
    for (AuraEffectList::const_iterator i = mDamageDone.begin(); i != mDamageDone.end(); ++i)
        if (((*i)->GetMiscValue() & schoolMask) != 0 && (*i)->GetSpellInfo()->EquippedItemClass == -1)
...
bool Unit::isSpellCrit(Unit* victim, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType) const
{
    ...
    switch (spellProto->DmgClass)
    {
                ...
                switch (spellProto->SpellFamilyName)
                {
                    ...
                    case SPELLFAMILY_PRIEST:
                        ...
                        if (AuraEffect * aurEff = GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_PRIEST, 329, 0))
                            if (spellProto->Id == 2061 || spellProto->Id == 2060 || spellProto->Id == 47540)
            ...
            if (victim)
            {
                ...
                switch (spellProto->SpellFamilyName)
                {
                    case SPELLFAMILY_DRUID:
                        ...
                        if ((spellProto->Id == 6807 || spellProto->Id == 5221) && victim->HasAuraState(AURA_STATE_BLEEDING))
                        {
                    ...
                    case SPELLFAMILY_HUNTER:
                        ...
                        if (spellProto->Id == 83381)
                            if (Unit *owner = GetOwner())
                                if (AuraEffect const * aurEff = owner->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_HUNTER, 2221, 0))
                                    crit_chance += aurEff->GetAmount();
                    break;
                }
            }
        ...
        case SPELL_DAMAGE_CLASS_RANGED:
        {
            ...
            {
                //Careful Aim 
                if (spellProto->Id == 19434 || spellProto->Id == 82928 || spellProto->Id == 77767 || spellProto->Id == 56641)
                    if (AuraEffect const *aurEff = GetDummyAuraEffect(SPELLFAMILY_HUNTER, 2222, 0))
                        if (victim->HealthAbovePct(90))
                            crit_chance += aurEff->GetAmount();
    ...
    // Curse of guldan
    if (IsPet() && GetOwner()->getClass() == CLASS_WARLOCK)
        if (victim->HasAura(86000, GetOwner()->GetGUID()))
            crit_chance += 10;
    ...
    {
        SpellReceivedCriticalValue(victim, spellProto);
        return true;
    }
    return false;
}

// We need this function because some spell don't wanna proc like other spells
void Unit::SpellReceivedCriticalValue(Unit* victim, const SpellInfo* spellProto) const
{
    switch (spellProto->SpellFamilyName)
    {
       case SPELLFAMILY_WARLOCK:
            //Mana feed
           if ((spellProto->Id == 3110 ||spellProto->Id == 54049 ||spellProto->Id == 30213 ||spellProto->Id == 7814 ||spellProto->Id == 3716) && IsPet())
               if (AuraEffect* pAurEff = GetOwner()->GetDummyAuraEffect(SPELLFAMILY_WARLOCK, 1982, 2))
               {
                   int32 bp = pAurEff->GetAmount();
                   GetOwner()->CastCustomSpell(GetOwner(), 91706, &bp, NULL, NULL, true);
               }
            break;
       case SPELLFAMILY_MAGE:
           if (spellProto->Id == 116)
       ...
       case SPELLFAMILY_HUNTER:
           // Invigoration
           if (spellProto->Id == 16827 || spellProto->Id == 49966 || spellProto->Id == 17253)
               if (AuraEffect* pAurEff = GetOwner()->GetDummyAuraEffect(SPELLFAMILY_HUNTER, 3487, 0))
               {
                   int32 bp = pAurEff->GetAmount();
                   GetOwner()->CastCustomSpell(GetOwner(), 53398, &bp, NULL, NULL, true);
               }
    }
}
...
uint32 Unit::SpellHealingBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack) const
{
    ...
    {
        // Selfes healer
        if (AuraEffect * aurEff = GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_PALADIN, 3924, 0))
            if (victim->GetGUID() != GetGUID())
                AddPct(DoneTotalMod, aurEff->GetAmount());
    ...
    // Nature's Blessing
    if (damagetype != DOT)
        if (AuraEffect * aurEff = GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_SHAMAN, 2012, 0))
            AddPct(DoneTotalMod, aurEff->GetAmount());
    ...
    // Healing done percent dependet on has aura
    AuraEffectList const& mHealingDonePctOnHasAura = GetAuraEffectsByType(SPELL_AURA_359);
    for (AuraEffectList::const_iterator i = mHealingDonePctOnHasAura.begin(); i != mHealingDonePctOnHasAura.end(); ++i)
        if ((*i)->GetId() == 99263)
            if (victim && victim->HasAura(99252))
                AddPct(DoneTotalMod, (*i)->GetAmount());
...
int32 Unit::ModifyPower(Powers power, int32 dVal)
{
    ...
    if (val <= 0 && power != POWER_ECLIPSE)
    {
        SetPower(power, 0);
        return -curPower;
    }
    else if (val <= -100 && power == POWER_ECLIPSE)
    {
        SetPower(power, -100);
        return -curPower;
    }
...
uint32 Unit::GetPowerIndex(uint32 powerType) const
{
    ...
    //Bot: another hack is for npc_bots
...
int32 Unit::GetCreatePowers(Powers power) const
{
    switch (power)
    {
        ...
        case POWER_RUNES:
            return 0;
        case POWER_SOUL_SHARDS:
            return (GetTypeId() == TYPEID_PLAYER && getClass() == CLASS_WARLOCK ? 3 : 0);
        case POWER_ECLIPSE:
            return (GetTypeId() == TYPEID_PLAYER && getClass() == CLASS_DRUID ? 100 : -100);
        case POWER_HOLY_POWER:
            return (GetTypeId() == TYPEID_PLAYER && getClass() == CLASS_PALADIN ? 3 : 0);
        case POWER_ALTERNATE_POWER:
            return 100;
        case POWER_HEALTH:
            return 0;
    ...
    for (AuraApplicationMap::const_iterator itr = GetAppliedAuras().begin(); itr!= GetAppliedAuras().end(); ++itr)
    {
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            if (itr->second->HasEffect(i))
            {
                ...
                if (!isTriggerAura[aurEff->GetAuraType()] && triggerData.spellProcEvent == NULL && spellProto->Id != 85767 && spellProto->Id != 85768)
                    continue;
