...
{
	#ifdef ELUNA
		sEluna->OnMoneyChanged(player, (int32&)amount);
	#endif
...
void ScriptMgr::OnPlayerUpdateZone(Player* player, uint32 newZone, uint32 newArea, uint32 oldZone, uint32 oldArea)
{
	#ifdef ELUNA
		sEluna->OnUpdateZone(player, newZone, newArea, oldZone, oldArea);
	#endif
    FOREACH_SCRIPT(PlayerScript)->OnUpdateZone(player, newZone, newArea, oldZone, oldArea);
}
...
{
	#ifdef ELUNA
		sEluna->OnMemberWitdrawMoney(guild, player, (uint32&)amount, isRepair);
	#endif
...
{
	#ifdef ELUNA
		sEluna->OnMemberDepositMoney(guild, player, (uint32&)amount);
	#endif
